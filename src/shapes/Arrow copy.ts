 ...
 */
      ...
      start: new Control({
        ...
        actionHandler: (eventData, transform, x, y) => {
          ...
          if (hoverTarget) {
            ...
            const minPoint = calcDistanceToTarget({ x, y }, hoverTarget);
      ...
      acc[i] = new Control({
        ...
        actionHandler: (eventData, transform, x, y) => {
          ...
          if (hoverTarget) {
            ...
            const oldConnObj =
              target.connectorEnd && target.connectorEnd._id
                ? canvas.findById(target.connectorEnd._id)
                : null;
  ...
  _render(ctx) {
    ...
    ctx.lineWidth = (this.strokeWidth * 1) / Math.max(canvas.getZoom(), 1);
    ctx.strokeStyle = this.stroke;
    ctx.fillStyle = this.stroke;
    ctx.lineJoin = 'round';
    ctx.lineCap = 'round';

    const connStartObj = canvas.findById(this.connectorStart?._id);
    const connEndObj = canvas.findById(this.connectorEnd?._id);
  ...
  drawBeizerBody(
    ...
    offset = 20;
    let start, cp1, cp2, end, newstart, newend, fPoints;
  ...
  mousedownProcess(transformData, eventData, isStart) {
    const { target, corner } = transformData;
    if (target.locked) return;
  ...
  mouseupProcess(transformData, eventData, isStart) {
    ...
    const targetObject = canvas.getTopObjectByPointer(pointer, true, true);
    if (target.connectorStart) {
      oldConnStartObj = canvas.findById(target.connectorStart._id);
      connStartId = target.connectorStart._id;
    }
    if (target.connectorEnd) {
      oldConnEndObj = canvas.findById(target.connectorEnd._id);
